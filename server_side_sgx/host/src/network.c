...
network *load_network_layerwise(char *cfg, char *file_buf, int index_layer)
{
    network *net = parse_network_cfg(cfg);

    // load all trained layers weights
    for(int idx_l=1; idx_l<=index_layer; idx_l++)
    {
        char weights[256];
        sprintf(weights, "%s%d_tee", file_buf, idx_l);

        if(weights && weights[0] != 0){
            load_weights_layerwise(net, weights, idx_l);
        }
    }
