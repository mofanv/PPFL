...
void load_weights_separate(network *net, char *filename0, int start, int cutoff)
{
    ...
    for(int i = start; i < net->n && i < cutoff; ++i){
        ...
        if(i > partition_point1 && i <= partition_point2){
            load_weights_layer(l, fp_tee, transpose_tee);
        }
    }
    fclose(fp_tee);
    free(filename_tee);
...
void load_weights_layerwise(network *net, char *filename, int idx_layer)
{
        fprintf(stderr, "Loading weights from %s...", filename);
        fflush(stdout);
        FILE *fp = fopen(filename, "rb");
        ...
        layer l = net->layers[idx_layer-1];
        load_weights_layer(l, fp, transpose);

        fprintf(stderr, "Done with layer %d!\n", idx_layer);
        fclose(fp);
}
